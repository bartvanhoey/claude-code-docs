...
2. **Code Quality (D? → B): split the 3 overlong functions:** ✅ Fixed 2026-07-19 — all three now under 100 lines: `read_doc()` 103→65, `safe_git_update()` 144→94 (two extractions needed), `main()` 183→86 (two extractions needed). Verified via git-repo simulations (bash) and mock-based tests (Python); see commits on `fix/health-audit-2026-07-19`. Two unrelated pre-existing bugs found along the way, logged as findings 5 and 6 below rather than fixed in this pass.
...
6. **`install.sh:safe_git_update()` — fast-path `git pull` can leave a locally-modified tracked file out of sync with the new HEAD:** verified with a real git repo pair: a locally-modified `docs/docs_manifest.json` (the normal state after a docs sync) plus an upstream change to the same file causes `git pull` to report success and advance `HEAD` to the new commit, but the working-tree file keeps the local uncommitted content — `git status` then shows it as "modified" relative to a HEAD it was never actually reconciled against. Not a crash and self-corrects on a subsequent run (which hits the already-fixed fallback block), but the inconsistency window itself wasn't previously known. Discovered 2026-07-19 alongside finding 5, same session; not fixed. Flagged here for investigation — worth understanding whether git's fast-forward semantics are being relied on incorrectly here before choosing a fix.
   Estimated effort: unestimated — needs investigation before a fix can be scoped.
